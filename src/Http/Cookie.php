...
class Cookie
{
    ...
    public static function fromString(string $cookie): self
    {
        ...
        if (false === strpos($cookieParts[0], '=')) {
            throw new InvalidArgumentException('Invalid cookie! Cookie must contain a name-value pair.');
        }
        ...
        if ($attributes['Expires']) {
            ...
            foreach ($expiresParts as $expiresPart) {
                if (! $time && preg_match('/^(\d{1,2})\:(\d{1,2})\:(\d{1,2})$/', $expiresPart, $matches)) {
                    $time = [
                ...
                if (! $day && preg_match('/^(\d{1,2})$/', $expiresPart, $matches)) {
                    $day = (int) $matches[1];

                    continue;
                }

                if (! $month && preg_match('/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)$/i', $expiresPart, $matches)) {
                ...
                if (! $year && preg_match('/^(\d{2,4})$/', $expiresPart, $matches)) {
                    $year = (int) $matches[1];
